...
bool MyButton(const char* label, const ImVec2& size)
{
	ImVec2 textSize = ImGui::CalcTextSize(label);
	ImVec2 textPosition = ImVec2((size.x - textSize.x) * 0.5f, (size.y - textSize.y) * 0.5f);
	ImGui::PushStyleVar(ImGuiStyleVar_ChildBorderSize, 12.0f); // Adjust rounding if needed
...
void gui::Render() noexcept
{
 ...
 style.Colors[ImGuiCol_CheckMark] = ImVec4(0.72, 0.05, 0.14, 1.0);  // Change checkmark color
 ...
 ImGui::Begin(
		"Abyss Menu + | itssnowww ",
		&isRunning
	);
 ImGui::Text("FPS: %.0f FPS", ImGui::GetIO().Framerate, (ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoBackground));
 ...
 if (MyButton("Cool Button", buttonSize))
 {
	 // Handle button click
	 // This code will be executed when the button is clicked
 }
